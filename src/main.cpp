...
unsigned int nStakeTargetSpacing = 60; // 60 seconds
unsigned int nStakeMinAge = 2 * 24 * 60 * 60; // 48 hours
...
const string strMessageMagic = "SpaceCash Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
    ...
    else if (pindexBest->nHeight <= 23040)
      {
        int64_t nSubsidy = 8 * COIN;
        return nSubsidy + nFees;
      }      
       
    else if (pindexBest->nHeight <= 25920)
      {
        int64_t nSubsidy = 9 * COIN;
        return nSubsidy + nFees;
      }      
       
    else if (pindexBest->nHeight <= 28800)
      {
        int64_t nSubsidy = 10 * COIN;
        return nSubsidy + nFees;
      }      
       
    else if (pindexBest->nHeight <= 31680)
      {
        int64_t nSubsidy = 11 * COIN;
        return nSubsidy + nFees;
      }      
       
    else if (pindexBest->nHeight <= 34560)
      {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SpaceCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
